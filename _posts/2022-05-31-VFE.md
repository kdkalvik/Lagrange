...
[Gaussian processes](https://itskalvik.github.io/GPs) are one of the most, if not the most, mathematically beautiful and elegant machine learning methods in history. We can use them for classification, regression, or generative problems. Also, the best part, they are probabilistic, so we can quantify the uncertainty in our predictions and have a lower risk of overfitting. 
...
&= c \exp{ \left(-\frac{1}{2} \left[ -2\underbrace{\frac{1}{\sigma^2} \mathbf{y}^\top \mathbf{K}_{fu}\mathbf{K}_{uu}^{-1}}_{b^\top}\mathbf{u} + \mathbf{u}^\top(\underbrace{\mathbf{K}_{uu}^{-1} + \frac{1}{\sigma^2}\mathbf{K}_{uu}^{-1}\mathbf{K}_{uf}\mathbf{K}_{fu}\mathbf{K}_{uu}^{-1}}_{M})\mathbf{u} \right] \right) } \\
\end{aligned}
$$

where $$c$$ is a constant. [Completing the quadratic](https://davidrosenberg.github.io/mlcourse/Notes/completing-the-square.pdf) form we recognize the Gaussian
