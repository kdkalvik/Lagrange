...
&= c \exp{ \left( \frac{1}{2} \mathbf{u}^\top (\mathbf{K}_{uu}^{-1} + \frac{1}{2 \sigma^2} \mathbf{K}_{uu}^{-1}\mathbf{K}_{uf}\mathbf{K}_{fu}\mathbf{K}_{uu}^{-1}  ) \mathbf{u} + \frac{1}{2 \sigma^2} \mathbf{y}^\top \mathbf{K}_{fu} \mathbf{K}_{uu}^{-1} \mathbf{u} \right)}
\end{aligned}
$$
...
* [Why the variational approach is a big deal](https://www.secondmind.ai/labs/sparse-gps-approximate-the-posterior-not-the-model/)
